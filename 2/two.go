...
func score_round(opponent_move string, my_move string) (opponent_score int, my_score int) {
	...
	inverted_rank_map := map[string]string{
		"scissors": "rock",
		"paper":    "scissors",
		"rock":     "paper",
	}
	...
	if my_move_translated == "lose" {
		my_score = 0 + choice_score_map[rank_map[opponent_move_translated]]
		opponent_score = 6 + choice_score_map[opponent_move_translated]
	} else if my_move_translated == "draw" {
		my_score = 3 + choice_score_map[opponent_move_translated]
		opponent_score = 3 + choice_score_map[opponent_move_translated]
	} else if my_move_translated == "win" {
		my_score = 6 + choice_score_map[inverted_rank_map[opponent_move_translated]]
		opponent_score = 0 + choice_score_map[opponent_move_translated]
	} else {
		panic("invalid coniditon met")
	}
